...
class CoolerPopsicle(modEventBus: IEventBus, modContainer: ModContainer) {
    companion object{
        ...
        // Create a Deferred Register to hold Blocks which will all be registered under the "cooler_popsicle" namespace
        val BLOCKS: DeferredRegister.Blocks = DeferredRegister.createBlocks(MOD_ID)
        // Create a Deferred Register to hold Items which will all be registered under the "cooler_popsicle" namespace
        val ITEMS: DeferredRegister.Items = DeferredRegister.createItems(MOD_ID)
        // Create a Deferred Register to hold CreativeModeTabs which will all be registered under the "cooler_popsicle" namespace
        val CREATIVE_MODE_TABS: DeferredRegister<CreativeModeTab> = DeferredRegister.create(Registries.CREATIVE_MODE_TAB, MOD_ID)

        // Creates a new Block with the id "cooler_popsicle:example_block", combining the namespace and path
        val EXAMPLE_BLOCK: DeferredBlock<Block> = BLOCKS.registerSimpleBlock(
                "example_block",
                BlockBehaviour.Properties.of().mapColor(MapColor.STONE)
            )

        // Creates a new BlockItem with the id "cooler_popsicle:example_block", combining the namespace and path
        val EXAMPLE_BLOCK_ITEM: DeferredItem<BlockItem> =
            ITEMS.registerSimpleBlockItem("example_block", EXAMPLE_BLOCK)

        // Creates a new food item with the id "cooler_popsicle:example_id", nutrition 1 and saturation 2
        ...
        // Creates a creative tab with the id "cooler_popsicle:example_tab" for the example item, that is placed after the combat tab
        val EXAMPLE_TAB: DeferredHolder<CreativeModeTab, CreativeModeTab> =
            CREATIVE_MODE_TABS.register("example_tab", Supplier {
                CreativeModeTab.builder()
                    .title(Component.translatable("itemGroup.cooler_popsicle")) //The language key for the title of your CreativeModeTab
